...
class LaporanActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val pemanfaatanRepository = PemanfaatanRepositoryImpl(apiService)
        financialViewModel = FinancialViewModel(pemanfaatanRepository)
        ...
        // Load financial data with network connectivity checks and retry logic
        loadFinancialDataWithRetry()
    }
    
    private fun loadFinancialDataWithRetry(currentRetryCount: Int = 0) {
                ...
                if (response.isSuccessful) {
                    ...
                    if (currentRetryCount < maxRetries) {
                        Handler(Looper.getMainLooper()).postDelayed({
                            loadFinancialDataWithRetry(currentRetryCount + 1)
