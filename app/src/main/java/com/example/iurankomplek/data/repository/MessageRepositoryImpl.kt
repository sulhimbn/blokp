...
class MessageRepositoryImpl(
    ...
    override suspend fun getMessages(userId: String): Result<List<Message>> {
        ...
        return withCircuitBreaker { apiService.getMessages(userId) }
            .also { result ->
    ...
    override suspend fun getMessagesWithUser(receiverId: String, senderId: String): Result<List<Message>> {
        return withCircuitBreaker { apiService.getMessagesWithUser(receiverId, senderId) }
    }
    ...
    private suspend fun withCircuitBreaker(
        apiCall: suspend () -> retrofit2.Response<List<Message>>
    ): Result<List<Message>> {
        val circuitBreakerResult = circuitBreaker.execute {
            var currentRetry = 0
            var lastException: Exception? = null
